import {
  getDocument,
  GlobalWorkerOptions,
} from "https://unpkg.com/pdfjs-dist@{{.PdfJsVersion}}/build/pdf.min.mjs";
...
function deleteOrJump(elem) {
  if (deleteOnClickCheckBox.checked) {
    ...
    const fromPage = parseInt(
      document.getElementById(`${assignmentId}-from`).textContent,
    );
...
function addAssignment() {
  ...
  const assignmentDesc =
    (chosenInstrument.textContent || "") +
    (document.getElementById("part-number")?.value || "");
  const assignmentId = assignmentDesc.toLowerCase().replace(/\s+/g, "");

  if (!assignmentId) {
    alert(
      "Please select an instrument and enter a part number before assigning a page.",
    );
    return 1;
  }
  ...
  if (!assignmentDiv) {
    ...
    desc.className =
      "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded px-2 py-1 z-10 whitespace-nowrap";
    ...
    if (newTo >= currentFrom) {
      ...
      alert(
        "Attempting to assign a page before the current first page. If you want to change the first page, please remove the assignment and reassign it.",
      );
      return 1;
    }
  }
...
function assignmentColor(assignmentId) {
  if (
    assignmentId.toLowerCase().includes("trumpet") ||
    assignmentId.toLowerCase().includes("cornet")
  ) {
    ...
    assignmentId.toLowerCase().includes("tuba") ||
    assignmentId.toLowerCase().includes("bass")
  ) {
...
async function submitPartitions() {
  ...
  for (const div of assignmentSection.children) {
    if (div.id.endsWith("-group")) {
      ...
      const fromPage = document.getElementById(
        `${assignmentId}-from`,
      ).textContent;
  ...
  if (!response.ok) {
    ...
    response.text().then((html) => {
      document.getElementById("flashMessage").innerHTML = html;
    });
  }
}
